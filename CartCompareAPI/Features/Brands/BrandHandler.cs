...
public sealed class BrandHandler(AppDbContext db)
{
    ...
    {
        Brand? brand = await db.Brands.FindAsync(id); if (brand is null)
        {
    ...
    public async Task<CrudResult> DeleteAsync(Guid id)
    {
        ...
        if (await db.Products.AnyAsync(x => x.BrandId == id))
        {
            return CrudResult.Conflict("This brand cannot be deleted because products reference it.");
        }
