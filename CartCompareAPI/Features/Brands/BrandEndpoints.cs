...
public static class BrandEndpoints
{
    ...
    {
        RouteGroupBuilder group = app.MapGroup("/api/brands").WithTags("Brands");
        group.MapGet("", async (BrandHandler handler) => Results.Ok(await handler.GetAllAsync())).WithName("GetBrands");
        group.MapGet("/{id:guid}", async (Guid id, BrandHandler handler) => { BrandResponse? brand = await handler.GetByIdAsync(id); return brand is null ? Results.NotFound() : Results.Ok(brand); }).WithName("GetBrandById");
        group.MapPost("", async (UpsertBrandRequest request, BrandHandler handler) => { CrudResult<BrandResponse> result = await handler.CreateAsync(request); return result.Error is not null ? Results.ValidationProblem(new Dictionary<string, string[]> { ["request"] = [result.Error] }) : Results.Created($"/api/brands/{result.Value!.Id}", result.Value); }).WithName("CreateBrand");
