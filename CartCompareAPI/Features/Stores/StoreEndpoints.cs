...
public static class StoreEndpoints
{
    ...
    {
        RouteGroupBuilder group = app.MapGroup("/api/stores").WithTags("Stores");
        ...
        {
            StoreResponse? store = await db.Stores.AsNoTracking().Where(x => x.Id == id)
        ...
        {
            IResult? error = await Validate(request, db);
        ...
        group.MapPut("/{id:guid}", async (Guid id, UpsertStoreRequest request, AppDbContext db) =>
        {
            ...
            IResult? error = await Validate(request, db, id);
            if (error is not null)
            {
                return error;
            }
